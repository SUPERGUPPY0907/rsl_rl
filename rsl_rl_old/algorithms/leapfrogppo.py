...
class LeapfrogPPO:
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            # -- actor
            actions_log_prob_batch = self.policy.actor.inverse(obs_batch, actions_batch)
            # -- critic
            ...
            if self.use_compress:
                actions = self.policy.actor.inference(obs_batch)
                velocity = actions[:, self.policy.actor.a_dim :]
                compress_loss = torch.norm(velocity, dim=1).mean()
            else:
                compress_loss = torch.zeros((), device=actions_log_prob_batch.device)
            ...
            if self.use_entropy:
                ...
                entropy = torch.zeros((), device=actions_log_prob_batch.device)
                loss_entropy = torch.zeros((), device=actions_log_prob_batch.device)
