    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            for i in range(n):
                ...
                # -- actor
                actions_log_prob_batch = self.policy.actor.inverse(obs_batch[i*batch:min((i+1)*batch, original_batch_size)], actions_batch[i*batch:min((i+1)*batch, original_batch_size)], jac=False)
                # -- critic
                ...
                if self.use_clipped_value_loss:
                    value_clipped = target_values_batch + (value_batch - target_values_batch[i*batch:min((i+1)*batch, original_batch_size)]).clamp(
