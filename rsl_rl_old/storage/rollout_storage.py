...
class RolloutStorage:
    ...
    def __init__(
        ...
        # for moser
        if training_type == "rl_moser":
            self.values = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
            self.actions_prob = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
            self.returns = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
            self.advantages = torch.zeros(num_transitions_per_env, num_envs, 1, device=self.device)
    ...
    def add_transitions(self, transition: Transition):
        ...
        # for moser
        if self.training_type == "rl_moser":
            self.values[self.step].copy_(transition.values)
            self.actions_prob[self.step].copy_(transition.actions_prob.view(-1, 1))
    ...
    def moser_mini_batch_generator(self, num_mini_batches, num_epochs=8):
        if self.training_type != "rl_moser":
            raise ValueError("This function is only available for moser.")
        batch_size = self.num_envs * self.num_transitions_per_env
        mini_batch_size = batch_size // num_mini_batches
        indices = torch.randperm(num_mini_batches * mini_batch_size, requires_grad=False, device=self.device)
        ...
        # For MoserPPO: log_prob here is prob
        old_actions_prob = self.actions_prob.flatten(0, 1)
        advantages = self.advantages.flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                target_values_batch = values[batch_idx]
                returns_batch = returns[batch_idx]
                old_actions_prob_batch = old_actions_prob[batch_idx]
                advantages_batch = advantages[batch_idx]

                # yield the mini-batch
                yield obs_batch, privileged_observations_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_prob_batch
